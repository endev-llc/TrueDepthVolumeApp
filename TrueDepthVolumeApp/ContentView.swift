            ...
            VStack {
                ...
                GeometryReader { geometry in
                    ZStack {
                        ...
                        // MOVE THE CONTOUR DRAWING HERE - inside the GeometryReader
                        if !cameraManager.detectedContour.isEmpty && !isDrawingMode {
                            Path { path in
                                let contour = cameraManager.detectedContour
                                guard !contour.isEmpty else { return }
                                
                                // Scale contour to match image frame
                                let scaledContour = contour.map { point in
                                    let x = imageFrame.minX + (CGFloat(point.x) / CGFloat(depthImage.size.width)) * imageFrame.width
                                    let y = imageFrame.minY + (CGFloat(point.y) / CGFloat(depthImage.size.height)) * imageFrame.height
                                    return CGPoint(x: x, y: y)
                                }
                                
                                path.move(to: scaledContour[0])
                                for point in scaledContour.dropFirst() {
                                    path.addLine(to: point)
                                }
                                path.closeSubpath()
                            }
...
class CameraManager: NSObject, ObservableObject, AVCaptureDepthDataOutputDelegate, AVCapturePhotoCaptureDelegate {
    ...
    @Published var detectedContour: [CGPoint] = []
    ...
    func processUploadedCSV(_ fileURL: URL) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            do {
                ...
                // Process with gradient segmentation
                self.processDepthDataWithGradientSegmentation(depthPoints)
    ...
    private func processDepthDataWithGradientSegmentation(_ depthData: [DepthPoint]) {
        // Convert to format expected by OpenCV processor
        let points = depthData.map { point in
            [NSNumber(value: point.x), NSNumber(value: point.y), NSNumber(value: point.depth)]
        }
        
        DepthGradientProcessor.processDepthData(points: points) { [weak self] edgeImage, contour in
            DispatchQueue.main.async {
